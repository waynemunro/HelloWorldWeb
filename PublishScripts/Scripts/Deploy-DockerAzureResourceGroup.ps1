...
Param(
  ...
  [string] $TemplateFile = "..\Templates\HelloWorldWebDockerVM.json",
  [string] $TemplateParametersFile = "..\Templates\HelloWorldWebDockerVM.param.dev.json",
...
function EnsureDockerCertificates($directory, $opensslExePath, $opensslConfigPath)
{
    ...
    "subjectAltName = IP:10.10.10.20,IP:127.0.0.1,DNS.1:*.cloudapp.net,DNS.2:*.*.cloudapp.azure.com" | Out-File extfile.cnf -Encoding ASCII 2>&1>$null
